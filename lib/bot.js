...
class Bot {
  constructor(options) {
    ...
    // Do not send messages starting with [off] to Discord
    this.doNotForwardOffMessages = options.doNotForwardOffMessages || true;
  ...
  ignoreIrcMessage(text) {
    return this.doNotForwardOffMessages && text.trimStart().substring(0, 5).toLowerCase() === '[off]';
  }
  ...
  sendToDiscord(author, channel, text) {
    ...
    // Do not send to Discord if the message wants to be ignored
    if (this.ignoreIrcMessage(text)) {
      return;
    }
